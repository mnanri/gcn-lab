...
def transform_to_fourier_with_rotate(dataset, graphs_dir, node_features_dir):
  data = 0
  with gzip.open(dataset, 'rb') as f:
    ...
    for i in range(int(3*len(_dataset)/5)):
      tmp_data = np.zeros((32,32), dtype=np.complex128)
      if i%3 == 0:
        for j in range(32):
          for k in range(32):
            tmp_data[31-k][j] = _dataset[i][j][k]
      elif i%3 == 1:
        for j in range(32):
          for k in range(32):
            tmp_data[k][31-j] = _dataset[i][j][k]
      else:
        for j in range(32):
          for k in range(32):
            tmp_data[31-j][31-k] = _dataset[i][j][k]
      _dataset[i] = tmp_data
  ...
  for e,imgtmp in enumerate(data):
    ...
    for i in range(2,34):
      for j in range(2,34):
        ...
        if np.abs(np.arctan2(i-cy,j-cx)) > np.arctan2(1,0):
          features[filter[12]][1] = np.abs(np.arctan2(i-cy,j-cx)) - np.arctan2(1,0)
        else:
          features[filter[12]][1] = np.abs(np.arctan2(i-cy,j-cx))
...
transform_to_fourier_with_rotate(dataset_dir + '/train-images-idx3-ubyte.gz', 'train_rotate_fourier_graphs', 'train_rotate_fourier_node_features')
transform_to_fourier_with_rotate(dataset_dir + '/t10k-images-idx3-ubyte.gz', 'test_rotate_fourier_graphs', 'test_rotate_fourier_node_features')
